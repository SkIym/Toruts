...
const TutorConfirmationForm = ({ tutor }: Props) => {
    ...
    const [subject, setSubject] = useState<string>(studentData.areasOfImprovement[0])

    const handleApply = async () => {
        if (user)
            try {
                console.log(tutor.id, subject, tutor.price)
                await dispatch(matchWithTutor(
                    user?.userName,
                    {
                        tutorId: tutor.id,
                        subject,
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-center">
                        {tutor.firstName} {tutor.lastName}'s
                        Tutoring Service
                    </DialogTitle>
                </DialogHeader>
                <DialogDescription className="flex flex-col">
                    ...
                    <div className="border-2 p-4 rounded-lg">
                        <p>
                            ...
                            talked to Mr. {tutor.lastName} using the
